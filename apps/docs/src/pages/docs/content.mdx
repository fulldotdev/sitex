...
export default function BlogPostLayout({
  ...
  return (
    <article>
      ...
      <p>
        During builds, Sitex also writes generated TSX mirrors for MDX pages
        under <code>.sitex/typecheck</code>. Those files import the selected
        layout and spread the frontmatter into it, so <code>tsc</code> reports
        the same prop errors you would get from a hand-written TSX route.
      </p>
...
explicit.
      ...
      <p>
        Sitex has two TypeScript surfaces for page data. Generated module types
        describe what <code>getPages</code> and <code>getPage</code> return.
        Generated MDX mirrors check frontmatter against the selected layout
        component props.
      </p>
      <CodeBlock
        lang="ts"
...
tags?: string[]
      ...
      />
      <p>
        In TSX routes, use <code>satisfies</code> when you want to enforce a
        consistent shape for an exported <code>data</code> object while
        preserving the inferred values Sitex reads.
      </p>
      <CodeBlock
        lang="ts"
        code={`type PostContent = {
...
export const data = {
  ...
  tags: ["react", "content"],
      ...
      />
      <p>
        In MDX routes, put that authoring contract on the layout props instead.
        The frontmatter is checked by passing it to the actual layout component
        in the generated typecheck mirror.
      </p>
